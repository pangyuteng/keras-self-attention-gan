...
class SAGAN():
    def __init__(self):
        ...
        self.channels = 1
        self.img_shape = (self.img_rows, self.img_cols, self.img_deps, self.channels)
        self.latent_dim = 100
    ...
    def build_generator(self):
        ...
        model.add(Conv3DTranspose(16,3,strides=(2,2),padding="same"))
        model.add(LeakyReLU(alpha=0.2))
        model.add(BatchNormalization(momentum=0.8))
        
        model.add(Conv3D(1,(3,3),padding="same",activation='tanh'))
        model.summary()
    ...
    def build_discriminator(self):
        ...
        model = Sequential()
        model.add(Conv3D(16,(3,3),padding="same",input_shape=self.img_shape))
        model.add(LeakyReLU(alpha=0.2))
        model.add(MaxPooling2D(pool_size=(2, 2)))

        model.add(self.discriminator_attn)

        model.add(Conv3D(32,(3,3),padding="same"))
        model.add(LeakyReLU(alpha=0.2))
        model.add(MaxPooling2D(pool_size=(2, 2)))
