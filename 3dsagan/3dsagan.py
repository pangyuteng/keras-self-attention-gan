...
"""
class SelfAttention3D(keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.conv_f = sn(layers.Conv3D(self.channel // 8, **mykwargs))# [bs, h, w, c']
        self.conv_g = sn(layers.Conv3D(self.channel // 8, **mykwargs)) # [bs, h, w, c']
        self.conv_h = sn(layers.Conv3D(self.channel, **mykwargs)) # [bs, h, w, c]
        self.conv_v = sn(layers.Conv3D(self.channel, **mykwargs))# [bs, h, w, c]
...
class SAGAN():
    ...
    def build_generator(self):
        ...
        self.generator_attn = SelfAttention3D(32)
        x, beta = self.generator_attn(x)
        x = Conv3DTranspose(16,(3,3,3),strides=(2,2,2),padding="same")(x)
        x = LeakyReLU(alpha=0.2)(x)
        x = BatchNormalization(momentum=0.8)(x)
        outputs = Conv3D(1,(3,3,3),padding="same",activation='tanh')(x)
    ...
    def build_discriminator(self):
        ...
        self.discriminator_attn = SelfAttention3D(16)
    ...
    def train(self, epochs, batch_size=128, sample_interval=50):
        ...
        # Load the dataset
        X_train = np.random.normal(0.5,0.25, (1000,28,28,28))
        X_train = X_train.clip(0,1)*255
        ...
        for i in range(r):
            for j in range(c):
                ...
                for n, source in enumerate(attn_dict.keys()):                    
                    x0,y0,z0 = source
                    x0,y0,z0 = float(x0)*SCALE,float(y0)*SCALE,float(z0)*SCALE # rescale to original image size
                    axs[i,j].scatter(y0,x0,s=0.8,color=COLOR_LIST[n],alpha=1)
                    target_list = attn_dict[source]
                    for target in target_list:
                        x1,y1,z1 = target
                        x1,y1,z1 = float(x1)*SCALE,float(y1)*SCALE,float(z1)*SCALE
